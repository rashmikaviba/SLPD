...
router.get('/pending', async (req, res) => {
  try {
    const pendingExpenses = await Expenses.find({ status: 'pending' });
...
router.put('/changeStatus/:id', async (req, res) => {
  const id = req.params.id;
  const { status } = req.body; // Corrected to use `status` instead of `newStatus`
  ...
  try {
    ...
    expense.status = status; // Updated to use `status`

...
function ExpenseList() {
    ...
    const approvedExpensesSum = expenses
        .filter(expense => expense.status === 'approve')
        .reduce((acc, curr) => acc + parseFloat(curr.Amount), 0);

    // Calculate the remaining balance
    const remainingBalance = balance - approvedExpensesSum;
    ...
    return (
        <div className='d-flex vh-100 bg-white justify-content-center align-items-center'>
            <div className='w-50 bg-primary rounded p-3'>
                <div>
                <div style={{ textAlign: 'right', marginBottom: '10px', fontSize: '1.25rem' }}>Price: 100000</div>
                <div style={{ textAlign: 'right', marginBottom: '10px', fontSize: '1.25rem' }}>Balance: {remainingBalance}</div>
